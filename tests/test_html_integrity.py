...
class TestHtmlIntegrity(unittest.TestCase):
    ...
    def test_portfolio_chat_has_canonical_endpoint_fallback(self):
        root = os.path.join(os.path.dirname(__file__), '..')
        with open(
            os.path.join(root, 'market_scanner.py'),
            'r', encoding='utf-8',
        ...
        self.assertIn('DEFAULT_PORTFOLIO_CHAT_API_URL', scanner)
        self.assertIn(
    ...
    def test_watchlist_exposes_enhanced_options_and_portfolio_fit_details(self):
        ...
        self.assertIn("<details class='panel enhanced-panel enhanced-details'>", content)
        self.assertIn("<summary class='enhanced-collapse-summary'>", content)
        self.assertNotIn("<details open class='panel enhanced-panel", content)
        ...
        self.assertIn('<section class="overview-row">', content)
        self.assertIn("<details class='panel technical-panel'>", content)
        self.assertIn("<summary class='technical-compact-summary'>", content)
        self.assertIn("<div class='technical-summary'>", content)
        self.assertIn("<span class='technical-open-label'>", content)
        self.assertNotIn("<details open class='panel technical-panel'>", content)
    ...
    def test_buy_now_web_push_uses_firebase_without_server_secret(self):
        ...
        self.assertIn('marketScannerPortfolioAuthenticated', generator)
        self.assertIn('market-scanner:portfolio-authenticated', generator)
        self.assertNotIn('Copiază tokenul Firebase', generator)
        self.assertNotIn('copyFirebaseTokenButton', generator)
    ...
    def test_portfolio_ai_analysis_is_in_encrypted_payload(self):
        ...
        self.assertIn('parseTradevilleHistory', content)
        self.assertIn("label:'NAV Tradeville · '", content)
        self.assertIn("label:'NAV ajustat Tradeville · '", content)
        self.assertIn("label:'Cash Tradeville · '", content)
        self.assertIn("label:'Profit ajustat Tradeville · '", content)
